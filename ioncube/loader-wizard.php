...
function runtime_loading_is_possible() {
    return !((is_php_version_or_greater(5,2,5) ) || ini_get('safe_mode') || !ini_get('enable_dl') || !function_exists('dl') || function_is_disabled('dl') || threaded_and_not_cgi());
}
...
function unrecognised_inis_webspace($startdir)
{
    ...
    for ($seps = 0; $seps < $depth; $seps++) {
        $full_ini_loc = realpath($startdir . '/' . $rel_path) . "/" . $ini_name;
        if (file_exists($full_ini_loc) && $sys['PHP_INI'] != $full_ini_loc) {
            $ini_list[] = $full_ini_loc;
        }
...
function extra_page()
{
    ...
    $lines["OS extra security"] = possibly_se_linux()?"SELinux":"None";
